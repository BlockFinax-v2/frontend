...
class SecureStorage {
  ...
  setPassword(password: string): void {
    ...
    console.log('Password stored in session storage');
  }
  ...
  setSessionPrivateKey(privateKey: string): void {
    ...
    console.log('Private key stored in session for transaction signing');
  }
  ...
  private ensurePassword(): void {
    if (!this.password) {
      ...
      if (sessionPassword) {
        this.password = sessionPassword;
        console.log('Password restored from session storage');
      } else {
        console.log('No session password found');
      }
    }
  }
  ...
  decryptWithPasswordCheck(encryptedData: string): string {
    ...
    if (this.password) {
      try {
        ...
        console.warn('Current password failed, trying session key...');
      }
    }
    ...
    if (sessionKey) {
      try {
        ...
        console.warn('Session key decryption failed');
      }
    }
  ...
  loadWallet(): StoredWallet | null {
    try {
      ...
      if (!this.password) {
        console.error('No password set for decryption');
        return null;
      }
      
      const decryptedData = this.decrypt(encryptedWallet);
      const wallet = JSON.parse(decryptedData);
      console.log('Wallet loaded successfully:', { address: wallet.address, name: wallet.name });
      return wallet;
    } catch (error) {
